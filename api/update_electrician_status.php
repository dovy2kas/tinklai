...
if ($role !== 'ADMIN') respond(['ok'=>false,'error'=>'Prieiga draudžiama'], 403);
...
if ($id <= 0 || !in_array($action, ['approve','unapprove'], true)) {
  respond(['ok'=>false,'error'=>'Neteisingi parametrai'], 400);
}

$st = $mysqli->prepare("SELECT id, statusas FROM ElektrikoProfilis WHERE id = ? LIMIT 1");
$st->bind_param("i", $id);
$st->execute();
$cur = $st->get_result()->fetch_assoc();
$st->close();

if (!$cur) respond(['ok'=>false,'error'=>'Profilis nerastas'], 404);

if ($action === 'approve') {
  $newStatus = 'PATVIRTINTAS';
} else {
  $newStatus = 'ATMESTAS';
}

$u = $mysqli->prepare("UPDATE ElektrikoProfilis SET statusas = ? WHERE id = ? LIMIT 1");
...
respond(['ok'=>true,'id'=>$id,'statusas'=>$newStatus]);
